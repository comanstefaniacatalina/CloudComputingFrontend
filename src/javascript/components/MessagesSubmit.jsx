  ...
  const handleMessageSend = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        alert(
          `Your original messages was in ${response.data.translationData.originalLanguage}. \nMessage sent: ${response.data.translationData.translatedText}`
        );
        //alert("da")
      }
    } catch (error) {
        ...
        <div className="flex flex-wrap -mx-3 mb-6">
          ...
          <div className="w-full md:w-1/2 px-3">
            ...
            <input
              ...
              id="receiverCountry"
              type="text"
              placeholder="cecilia@stud.ase.ro"
            />
          </div>
          <div className="w-full md:w-1/2 px-3">
            <label
              className="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2"
              htmlFor="receiverMail"
            >
              <LocationMarkerIcon
                className="h-5 w-5 font-pink"
                aria-hidden="true"
